...
// Import the chatflow data (you can copy this from chatflow_complete.json)
const chatflow = {
  "flow_metadata": {
    ...
    "main_menu": {
      ...
      "sections": [
        ...
        },
        {
          ...
          "rows": [
            { "id": "menu_speak", "title_en": "Speak to Sales Engineer", "title_ar": "التحدث مع مهندس مبيعات" },
...
function formatMessage(step, language = 'en') {
  const content = step[`message_${language}`] || step[`body_${language}`] || step.message_en || step.body_en || '';
  ...
  if (step.sections) {
    step.sections.forEach(section => {
      section.rows.forEach(row => {
        items.push({ id: row.id, label: row[`title_${language}`] || row.title_en || row.title || row.id });
      });
    });
  }
  if (step.rows) {
...
function sendInteractiveButtons(client, chatId, step, language = 'en') {
  const buttons = step.buttons.map(btn => ({
    buttonId: btn.id,
    buttonText: { displayText: btn[`title_${language}`] || btn.title_en },
    type: 1
  }));

  const buttonMessage = {
    text: formatMessage(step, language),
    footer: 'Hi-Tech Industrial Group',
    buttons: buttons,
    headerType: 1
  };

  return client.sendMessage(chatId, buttonMessage);
}

function sendInteractiveList(client, chatId, step, language = 'en') {
  const sections = step.sections.map(section => ({
    title: section[`title_${language}`] || section.title_en,
  ...
  const listMessage = {
    text: formatMessage(step, language),
    footer: step[`footer_${language}`] || step.footer_en || 'Hi-Tech Industrial Group',
    title: step[`header_${language}`] || step.header_en || 'Menu',
    buttonText: step[`button_text_${language}`] || step.button_text_en || 'Select Option',
    sections: sections
  };

  return client.sendMessage(chatId, listMessage);
}
...
function handleUserMessage(client, message) {
  ...
  const handoffKeywords = chatflow.global_settings.human_handoff_keywords;
  if (handoffKeywords.some(keyword => userInput.includes(keyword))) {
    updateUserSession(userId, { currentStep: 'speak_engineer' });
    const step = chatflow.steps.speak_engineer;
    sendMessage(client, message.from, formatMessage(step, session.language));
    return;
  }

  // Handle different step types
  const currentStep = chatflow.steps[session.currentStep] || chatflow.fallback_handlers[session.currentStep];

  if (!currentStep) {
    sendMessage(client, message.from, 'Sorry, there was an error. Let me restart your session.');
    updateUserSession(userId, { currentStep: 'welcome' });
    const welcomeStep = chatflow.steps.welcome;
    sendInteractiveButtons(client, message.from, welcomeStep, session.language);
    return;
  }

  // Handle button/list selections
  if (currentStep.type === 'interactive_button') {
    const buttonIds = currentStep.buttons.map(btn => btn.id);
    if (buttonIds.includes(userInput)) {
      const nextStepId = resolveNextStep(currentStep, userInput);
      if (nextStepId) {
        updateUserSession(userId, { currentStep: nextStepId });
        processStep(client, message.from, nextStepId, session.language);
      } else {
        sendMessage(client, message.from, 'Invalid selection. Please try again.');
      }
    } else {
      sendMessage(client, message.from, 'Please select one of the button options above.');
    }
  } else if (currentStep.type === 'interactive_list') {
    const allRowIds = flattenMenuOptions(currentStep).map(item => item.id);
    if (allRowIds.includes(userInput)) {
      const nextStepId = resolveNextStep(currentStep, userInput);
      if (nextStepId) {
        updateUserSession(userId, { currentStep: nextStepId });
        processStep(client, message.from, nextStepId, session.language);
      ...
      sendMessage(client, message.from, 'Please select from the list options above.');
    }
  } else if (currentStep.type === 'text') {
    // For text steps (forms), accept any input and move to next step
    const nextStepId = resolveNextStep(currentStep);
    if (nextStepId) {
      updateUserSession(userId, { currentStep: nextStepId });
      processStep(client, message.from, nextStepId, session.language);
    } else {
      // If no next step, go back to main menu
      updateUserSession(userId, { currentStep: 'main_menu' });
      processStep(client, message.from, 'main_menu', session.language);
    }
  } else {
    // Fallback for unrecognized input
    const fallbackStep = chatflow.fallback_handlers.unrecognized_input;
    sendMessage(client, message.from, formatMessage(fallbackStep, session.language));
  }
}

function processStep(client, chatId, stepId, language = 'en') {
  const step = chatflow.steps[stepId] || chatflow.fallback_handlers[stepId];

  if (!step) {
    sendMessage(client, chatId, 'Sorry, this step is not available.');
    return;
  }

  if (step.type === 'interactive_button') {
    sendInteractiveButtons(client, chatId, step, language);
  } else if (step.type === 'interactive_list') {
    sendInteractiveList(client, chatId, step, language);
  } else if (step.type === 'text') {
    sendMessage(client, chatId, formatMessage(step, language));
  } else {
    sendMessage(client, chatId, formatMessage(step, language));
  }
}
...
client.on('ready', () => {
  ...
  console.log('Hi-Tech Industrial Group WhatsApp Flow is now active.');
  console.log('Send messages to test the flow.');
});
...
client.on('message', async (message) => {
  ...
  try {
    ...
    sendMessage(client, message.from, 'Sorry, there was an error processing your message. Please try again.');
  }
});
